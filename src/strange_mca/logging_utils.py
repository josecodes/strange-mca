...
class DetailedLoggingCallbackHandler(BaseCallbackHandler):
    ...
    def on_chain_start(
        ...
        node_name = (
            kwargs.get("tags", ["unknown_node"])[0]
            if kwargs.get("tags")
            else "unknown_node"
        )
        ...
        if "task" in inputs:
            task_preview = (
                inputs["task"][:50] + "..."
                if len(inputs["task"]) > 50
                else inputs["task"]
            )
            self.logger.debug(
                f"[CHAIN_START:{node_name}] Task for {node_name}: {task_preview}"
            )
    ...
    def on_chain_end(self, outputs: dict[str, Any], **kwargs: Any) -> None:
        ...
        if "responses" in outputs and isinstance(outputs["responses"], dict):
            for agent_name, response in outputs["responses"].items():
                ...
                self.logger.debug(
                    f"[CHAIN_END:{node_name}] Response from {agent_name}: {response_preview}"
                )
    ...
    def on_llm_start(
        ...
        self.logger.info(
            f"[LLM_START:{node_name}] LLM call started (run_id: {run_id_short})"
        )

        if prompts:
            prompt_preview = (
                prompts[0][:30] + "..." if len(prompts[0]) > 30 else prompts[0]
            )
    ...
    def on_llm_end(self, response: Any, **kwargs: Any) -> None:
        ...
        self.logger.info(
            f"[LLM_END:{node_name}] LLM response (run_id: {run_id_short}): {content_preview}"
        )
    ...
    def on_chat_model_start(
        ...
        **kwargs: Any,
        ...
        # Log at debug level (only shown in detailed mode)
        self.logger.debug(
            f"[CHAT_START:{node_name}] Chat model call started (run_id: {run_id_short})"
        )
        ...
        if messages:
            ...
            self.logger.debug(
                f"[CHAT_START:{node_name}] Messages: {message_count} total"
            )
    ...
    def on_chat_model_end(self, response: Any, **kwargs: Any) -> None:
        ...
        # Log at debug level (only shown in detailed mode)
        self.logger.debug(
            f"[CHAT_END:{node_name}] Chat model response (run_id: {run_id_short}): {content_preview}"
        )
    ...
    def on_tool_start(
        ...
        # Log at debug level (only shown in detailed mode)
        self.logger.debug(
            f"[TOOL_START:{node_name}] Tool '{tool_name}' started (run_id: {run_id_short})"
        )
    ...
    def on_tool_end(self, output: str, **kwargs: Any) -> None:
        ...
        # Log at debug level (only shown in detailed mode)
        self.logger.debug(
            f"[TOOL_END:{node_name}] Tool completed (run_id: {run_id_short})"
        )
    ...
    def on_agent_action(self, action: dict[str, Any], **kwargs: Any) -> None:
        ...
        # Log at debug level (only shown in detailed mode)
        self.logger.debug(
            f"[AGENT_ACTION:{node_name}] Agent taking action '{action_name}' (run_id: {run_id_short})"
        )
        ...
        if action_input:
            input_preview = (
                str(action_input)[:30] + "..."
                if len(str(action_input)) > 30
                else str(action_input)
            )
            self.logger.debug(
                f"[AGENT_ACTION:{node_name}] Action input: {input_preview}"
            )
    ...
    def on_agent_finish(self, finish: dict[str, Any], **kwargs: Any) -> None:
        ...
        # Log at debug level (only shown in detailed mode)
        self.logger.debug(
            f"[AGENT_FINISH:{node_name}] Agent finished (run_id: {run_id_short})"
        )
...
def setup_detailed_logging(
    level: int = None, log_level: str = "warn", only_local_logs: bool = False
) -> None:
    ...
    actual_level = (
        level if level is not None else log_level_map.get(log_level, logging.WARNING)
    )
    ...
    for logger_name in logging.root.manager.loggerDict:
        if (
            only_local_logs
            and logger_name != "strange_mca"
            and not logger_name.startswith("strange_mca.")
        ):
